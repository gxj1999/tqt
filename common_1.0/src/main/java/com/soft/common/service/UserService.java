...
@Service
public class UserService extends AbstractService<MLogin> {
    ...
    @SystemLog("用户登录")
    public ResultBean login(HttpServletRequest request, String loginId, String password) {
        MLogin mLogin = getUserByLoginId(null, loginId);
        if (mLogin == null) {
            return ResultBean.failure("您所输入的用户名不存在!", 2);
        }
        MUser cond = new MUser();
        cond.setUserId(mLogin.getUserId());
        cond.setCntrCd(mLogin.getCntrCd());
        cond.setDelFlg("0");
        MUser mUser = mUserDao.selectOne(cond);
        ...
        if (!StringUtils.nullToEmpty(StringUtils.MD5(password)).equals(mLogin.getPswd())) {
            mLogin.setPswdErrNum(mLogin.getPswdErrNum() == null ? 0 : mLogin.getPswdErrNum() + 1);
            CommonUtils.setCommonInfo(mLogin, request);
            mLoginDao.updateByPrimaryKey(mLogin);
            //若密码错误，则更新用户登录表的【密码输入错误次数】等字段，同时判断是否超出次数
            if (mLogin.getPswdErrNum() >=  5) {
                mUser.setDisableFlg("2");
                CommonUtils.setCommonInfo(mUser, request);
                mUserDao.updateByPrimaryKey(mUser);
                return ResultBean.failure("当前用户名密码已经输错超过五次,请联系管理员!", 3);
            }
            return ResultBean.failure("密码输入错误,请重新输入!", 3);
        }
